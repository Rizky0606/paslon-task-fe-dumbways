    ...
    <div className="w-full">
      ...
      <div className="grid px-14 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {users?.map((paslon: DataUsers) => {
          return (
            <div
              className="bg-slate-400 m-5 rounded-md hover:scale-110"
              key={paslon.id}
            >
      ...
      <div className="w-full flex flex-wrap">
        {/* COUNTER VOTES */}
        <div className="flex-1 m-10 bg-neutral-300 p-7 rounded-md">
          <div className="flex flex-col items-start">
            <p className="font-bold text-[30px] m-auto">Suara Saat Ini</p>
            <ul>
              {users?.map((paslon: DataUsers) => {
                return (
                  <li key={paslon.id} className="my-5 text-[20px] font-bold">
                    {paslon.name} :
                  </li>
                );
              })}
        ...
        {/* FORM VOTE */}
        <div className="flex-1 m-10 bg-neutral-300 rounded-md">
                ...
                return (
                  <div key={paslon.id}>
                    <label className="label cursor-pointer">
                      <span className="label-text">{paslon.name}</span>
                      <input
                        type="radio"
                        name="radio-10"
